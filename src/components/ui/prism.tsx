...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const render = t => {
      ...
      if (animationType === '3drotate') {
        const ts = time * TS;
        program.uniforms.uRot.value = setMat3FromEuler(ts*0.5, Math.sin(ts*0.6)*0.6, Math.sin(ts*0.4)*0.5, rotBuf);
      } else {
    ...
    startRAF();
    return () => { stopRAF(); ro.disconnect(); if (gl.canvas.parentElement === container) container.removeChild(gl.canvas); };
  ...
  return <div className="absolute inset-0 w-full h-full -z-10" ref={containerRef} />;
};
